...
def main(ticker, forecast_days):
    ...
    plt.plot(range(1, forecast_days + 1), forecast, label='XGBoost', color='green')
    plt.title(f"{ticker} Forecast for Next {forecast_days} Days (XGBoost)")
    ...
    if "DISPLAY" in os.environ:
        ...
        filename = f"xgboost_{ticker.lower()}_forecast.png"
        plt.savefig(filename)
        print(f"圖已儲存為 {filename}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Forecast future stock price using XGBoost")
