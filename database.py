...
class SupabaseManager:
    ...
    def __init__(self):
        ...
        if not HAS_SUPABASE:
            logger.warning("⚠ 警告: 未安裝 supabase 套件")
            return
            
        if not url or not key:
            logger.warning("⚠ 警告: Supabase URL 或 Key 未設定")
            return
    ...
    def save_predictions(self, index_name: str, predictions: List[Tuple], model_name: str, period: str):
        ...
        try:
            ...
            if "relation" in str(e) and "does not exist" in str(e):
                 logger.warning("💡 提示: 請確認您的 Supabase 資料庫中已建立 'predictions' 資料表。")
                 print("   您可以在 SQL Editor 執行以下指令:")
    ...
    def save_dual_strategy_results(self, index_name: str, results: dict, period: str = "6mo"):
        """
        保存雙軌策略結果到 Supabase predictions 表格（原表擴展版）
        ...
        需要先執行 supabase_add_columns.sql 擴展 predictions 表格：
        - strategy_type: 策略類型
...
class MySQLManager:
    ...
    def __init__(self):
        ...
        if not HAS_MYSQL and DatabaseConfig.USE_MYSQL:
            print("⚠ 警告: 未安裝 MySQL 連接器，MySQL 功能已停用")
...
def save_to_mongodb(index_name: str, stock_predictions: dict):
    ...
    """
    if not HAS_MONGO:
        print("MongoDB 功能未啟用或未安裝 pymongo")
        return
